...
- ИИ-сводка обсуждения под видео (темы, ключевые мнения, тональность, яркие комментарии) — стриминговая, результат появляется по мере генерации (SSE через `/api/chat`);
- поиск по комментариям — по тексту и по смыслу (LLM-ранжирование);
- превью видео (открывается в отдельной вкладке) и информация о канале (название, подписчики, аватар, ссылка);
- тема интерфейса (тёмная / светлая / как в системе);
- экспорт сводки в `.md`-файл.
...
- **URL сервера** — по умолчанию `http://localhost:8080` (локальный tg-saver-bot-cloud):
  ...
  ⚠️ С 2026-09-17 облачный бэкенд на Render **остановлен** (исчерпан free-лимит) — сейчас
  работает только локальный `http://localhost:8080`, его и используйте;
...
- `groq.js` — клиент прокси (`chat` для обычных вызовов, `chatStream` для SSE-стриминга, retry/backoff, сериализация через `SerialQueue`).
